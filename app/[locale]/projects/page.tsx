...
export async function generateMetadata({ params }: { params: { locale: SupportedLocale } }) {
...
export default async function ProjectsPage({ params }: { params: { locale: SupportedLocale } }) {
    // await params ობიექტის გამოყენებამდე
    const paramsObject = await params;
    const locale = paramsObject.locale;
    ...
    return (
        <div className="container mx-auto py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {projects.map((project) => {
                    // აქ სწორად მივუთითოთ ტიპი და პირდაპირ გამოვიყენოთ ქართული ვერსია თუ მოთხოვნილ ენაზე არ არის მონაცემები
