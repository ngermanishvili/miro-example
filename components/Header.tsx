...
const Header = () => {
  ...
  const switchLanguage = (langCode: string) => {
    ...
    // If we only have a locale
    if (segments.length === 1 && ["en", "ka", "ru"].includes(segments[0])) {
      router.push(`/${langCode}`);
      return;
    }
                ...
                <li key={item.id}>
                  {item.isRoute ? (
                    <Link
                      ...
                      className={`hover:text-gray-600 hover:scale-110 transition-all duration-200 ${
                    ...
                    >
                      {
                        item.translations[
                          locale as keyof typeof item.translations
                        ]
                      }
                    </Link>
                  ) : (
                ...
                {menuItems.map((item) => (
                  <li key={item.id}>
                    {item.isRoute ? (
                      <Link
                        href={getFullPath(item.path)}
                        className={`text-xl font-bold block hover:text-gray-600 hover:scale-105 transition-all duration-200 ${
                          isActivePath(item.path)
                            ? "text-black"
                            : "text-gray-500"
                        }`}
                ...
                <li className="pt-4 border-t border-gray-200">
                  ...
                  <div className="flex flex-col space-y-3">
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
                        className={`text-lg font-bold block text-left hover:scale-105 transition-all duration-200 ${
                          locale === lang.code
                            ? "text-blue-600"
