// components/movies/PlatformMovieSection.tsx
"use client";
...
interface PlatformMovieSectionProps {
    platformId: string;
    platformName: string;
    colorClass?: string;
}
...
const PlatformMovieSection: React.FC<PlatformMovieSectionProps> = ({
    platformId,
    platformName,
    ...
    const getKey = (pageIndex: number, previousPageData: MoviesApiResponse | null) => {
        if (previousPageData && !previousPageData.hasMore) return null;
        // ლიმიტი რამდენჯერ მეტი იქნება ვიდრე MOVIES_PER_ROW * INITIAL_ROWS
        const limit = MOVIES_PER_ROW * INITIAL_ROWS;
        return `/api/movies?page=${pageIndex + 1}&limit=${limit}&platform=${platformId}`;
    };

    const { data, error, size, setSize } = useSWRInfinite<MoviesApiResponse>(getKey, fetcher, {
...
export default PlatformMovieSection;